...
| ID | Слайс | Ветка | Acceptance | Status |
|---|---|---|---|---|
| P10-SERVE | stdlib loopback JSON над FTS | `feature/P10-serve` | `handle()` поиск «Волга»/«МВД»; bind `127.0.0.1:8099`; не-loopback отказ; `fts_match` зелёный | IN PROGRESS |
| P11-BOX | Dockerfile + compose.yaml + entrypoint | `feature/P11-compose` | file-contract: 3.12, `cap_drop: ALL`, `127.0.0.1:8099`; `.dockerignore` без шаблона | PENDING |
