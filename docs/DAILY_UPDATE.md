...
Wikidata: SPARQL `P17=Q159` за 48 ч — сигнал **перезапустить** `python scripts/seed_municipalities.py` / `seed_microtoponyms.py` / `seed_hodonyms.py` / `seed_cities.py` (перед CalVer-тегом или когда в allowlist появились новые P31). Не дельта `daily.py`. JSON запроса в git не класть. Daily остаётся `kind=none`.
