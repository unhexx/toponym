...
upsert по `wd:Q…`. Это не все 1 126 городов России (Wikidata, ruwiki 13.05.2026) и не полный ОКТМО
и не Росстат; без Крыма/новых субъектов. SPARQL JSON в git нет. Имя файла `cities-major` историческое.
Массовая вставка — `python scripts/seed_cities.py` (не `daily.py`).
Город без P131 не числится в Адыгее: у Хаджи-Тархана и Ожска parent и admin1 пустые.
Майкоп и Адыгейск остаются в Адыгее.
...
Q634099), P17=Q159, upsert по `wd:Q…`. Это не ОКТМО/ГАР и не все МО России (Wikidata, не Росстат); SPARQL JSON в git нет.
...
и не все улицы России (Wikidata, не ГКГН); SPARQL JSON в git нет.

| Таблица | Сколько строк | Примеры |
|---|---|---|
| `data/curated/cities-major.csv` | около 1.1 тыс. (Wikidata Q7930989, не Росстат) | Москва, Самара, Тобольск |
...
Это не все площади России (Wikidata) и не ОМК УМ Москвы. SPARQL JSON в git нет.
...
Площадь без P131 на Москву не получает родителя `wd:Q649`. Красная площадь в Москве остаётся.
Красная площадь (Курск) — пустой parent.
...
Дорога без одного субъекта не схлопывается в Адыгею: у Транссиба parent пустой, аббревиатура на месте.
БАМ остаётся в Амурской области. А159 остаётся в Адыгее.
...
Сёла повторно не выгружались: parent у строк уже есть. Хутора Q5084 по-прежнему нет.
...
(не `daily.py`). Волга и Дон — gold, родители не двигались.
Объект без разрешённого P131 — пустой parent, не Адыгея. Кубань и другие реки с P131
в Адыгее остаются (Wikidata, не ГКГН и не все реки России).

Оронимы (DEC-SEED-011) — реестр Wikidata (CC0): P31 гора Q8502, хребет Q46831,
вулкан Q8072, остров Q23442 и полуостров Q34763, P17=Q159, upsert по `wd:Q…`.
Остров/полуостров — `type_id=insulonym`. Запрос по одному VALUES-типу.
Это не ГКГН и не все горы России. SPARQL JSON в git нет.
Массовая вставка — `python scripts/seed_oronyms.py` (не `daily.py`).
Эльбрус, Уральские горы и Сахалин сохранены. Байкал в оронимах не живёт.
Гора без разрешённого P131 — пустой parent, не Адыгея. В Адыгее остаются Фишт, Оштен,
Пшеха-Су, Ассара и Скалистый хребет (Wikidata, не ГКГН и не все горы России).

ФОИВ — join `wd`, не harvest: `python scripts/join_foiv.py` (не `daily.py`).
P31 министерство Q4481741, федеральная служба Q4481675, федеральное агентство
Q14944295, P17=Q159; Q4481793/Q4481792 нет. Совпадение по `name_ru`/`abbr`
с меткой Wikidata. Id остаётся `foiv:{slug}`, `source_id=ukase-326`.
0/69 совпадений CSV не пишет. Колонка `wd` по-прежнему у 57 из 69 строк.
Это не все ФОИВ России (Wikidata; DEC-SEED-012).

Досинхронизация известных Q-id — `python scripts/sync.py --source wikidata` (dry-run)
и `--apply`. SPARQL `VALUES` только по строкам, которые уже в CSV: города, сёла,
гидронимы, оронимы, площади, дромонимы, МО, годонимы, агентства. `inserted` остаётся 0
(DEC-SEED-013): новый Q-id из ответа строкой не становится. Daily по-прежнему
`wikidata kind=none` и этот SPARQL не запускает. Покрытие то же: Wikidata, не Росстат и не ГКГН.
...
| `data/curated/oronyms-major.csv` | около 13 тыс. (Wikidata Q8502/Q46831/Q8072/Q23442/Q34763, не ГКГН) | Эльбрус, Уральские горы, Сахалин |
...
оронимы Wikidata P31=Q8502 / Q46831 / Q8072 / Q23442 / Q34763 (не ГКГН и не все горы России; DEC-SEED-011),
ФОИВ указа № 326 / № 522 (`foiv:{slug}`; колонка `wd` — join Wikidata
P31 Q4481741/Q4481675/Q14944295, не harvest и не все ФОИВ России),
смежные ведомства, золотые склонения к части из них,
реестр муниципалитетов, годонимов и микротопонимов Wikidata (исторический малый сид DEC-SEED-001),
площадей Wikidata P31=Q174782 (не все площади России в Wikidata; DEC-SEED-008),
