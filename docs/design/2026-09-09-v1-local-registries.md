...
- Full import of GeoNames `RU.zip` or ГАР/ФИАС (**DEC-DUMP-001**, frozen).
- Inserting unmapped GeoNames ids as `gn:{geonameId}` curated rows (**DEC-GN-001**, frozen).
- Autogenerated pymorphy/Natasha rows as gold declensions (**DEC-DECL-001**, frozen).
- Rewriting landed gold declensions to force unique `id` (P1 uniqueness is `(id, lemma)`; **DEC-DECL-002**, frozen).
...
- Vendoring the Agentix template tree (symlink SSOT only) (**DEC-AGENTIX-001**).
...
**DEC-DECL-002:** uniqueness is `(id, lemma)` as in `test_seeds.py` and `validate.py`. Schema `primaryKey` is `["id", "lemma"]`; field `id` is **not** `unique`. Do not rewrite gold to `foiv:mvd#head`. Duplicate `id` with different lemmas (abbr + full name) is required.
...
**DEC-GN-001 (next release, frozen):** GeoNames remains match-only. `stable_id: gn:{geonameId}` in `data/mappings/geonames.yaml` is the join key onto the curated `geonames` column, not a license to insert. Unmapped mods increment `skipped_unmapped`. `validate.py` rejects place `id` values that start with `gn:`.
