...
from scripts.lib.detectors import TIMEOUT_SEC, build_session  # noqa: E402
...
def _write_body(response: requests.Response, dest_file: Path) -> int:
...
def download(session: requests.Session, url: str, dest_file: Path) -> int:
    response: requests.Response = session.get(url, stream=True, timeout=TIMEOUT_SEC)
    try:
