#!/usr/bin/env python3
"""Harvest Russian hydronyms from Wikidata into hydronyms-major.csv.

Coverage is one SELECT: P31=Q23397 (lake) and P31=Q4022 (river, ruwiki),
P17=Q159, not GKGN. Seas Q165 stay via FILL_IF_EMPTY, not SPARQL VALUES.
If the query is too_large/timeout, incoming rivers are dropped; lakes and
seas remain. SPARQL JSON/CSV is not vendored; optional --from-csv/--from-json
...
ROOT = _ROOT
SPARQL_PATH = Path("data/raw/wikidata/hydronyms-ru.sparql")
HYDRO_REL = Path("data/curated/hydronyms-major.csv")
DECL_REL = Path("data/declensions/hydronyms-major.csv")
CITIES_REL = Path("data/curated/cities-major.csv")
REGIONS_REL = Path("data/curated/regions.csv")
HYDRO_SPECS: tuple[tuple[Path, int], ...] = (
    (REGIONS_REL, 0),
    (CITIES_REL, 1),
)
HYDRO_SKIP_RELS = (
...
HOP_BATCH = 80
GOLD_PARENT_IDS = frozenset({"wd:Q626", "wd:Q1229"})
__all__ = [
    "HYDRO_SKIP_RELS",
    "HYDRO_SPECS",
    "apply_harvest",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Сиды гидронимов из Wikidata SPARQL (не ГКГН; моря Q165 не VALUES)"
    )
    ...
    parser.add_argument(
        "--types",
        default="",
        help="подмножество P31 через запятую (Q23397,Q4022)",
    )
    parser.add_argument(
        "--shard-by-region",
        action="store_true",
        help="шардить реки Q4022 по субъектам regions.csv (wall_sec=0)",
    )
...
def load_main_query(path: Path) -> str:
    return load_main_query_required(
        path, required=("SELECT", "VALUES", "?type", "Q23397", "Q4022")
    )


def queries_for_p31(query: str) -> list[tuple[str, str]]:
    return queries_for_p31_last(query, last=(RIVER_P31,))


def load_parent_index(root: Path) -> ParentIndex:
    return load_parent_index_specs(root, HYDRO_SPECS)


def skip_ids(root: Path) -> dict[str, str]:
    return skip_ids_rels(root, HYDRO_SKIP_RELS)
...
def too_large_or_timeout(exc: BaseException) -> bool:
...
def type_id_of(rec: dict[str, Any]) -> str:
    p31 = _p31(rec)
    if RIVER_P31 in p31:
        return TYPE_BY_P31[RIVER_P31]
    if LAKE_P31 in p31:
        return TYPE_BY_P31[LAKE_P31]
    return ""


def drop_incoming_rivers(records: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]]:
    kept: dict[str, dict[str, Any]] = {}
    for qid, rec in records.items():
        p31 = _p31(rec)
        if RIVER_P31 in p31 and LAKE_P31 not in p31:
            continue
        kept[qid] = rec
    return kept
...
def _prefer_replaced_by(
    replaced: set[str], known_ids: set[str], hydro_ids: set[str]
) -> str:
    hits: list[str] = []
    for raw in replaced:
        ...
        if qid.startswith("wd:"):
            ...
            continue
        if ident in known_ids or ident in hydro_ids:
            hits.append(ident)
    if not hits:
        return ""
    preferred = [ident for ident in hits if ident in hydro_ids]
...
def _merge_parent_refresh(
    ...
    Fill-if-empty не затирает ложный iso:RU-AD. Волга и Дон не двигаются.
    """
    if (existing.get("id") or "").startswith("local:"):
        return inc
    if (existing.get("id") or "") in GOLD_PARENT_IDS:
        return inc
...
def apply_harvest(
    ...
    *,
    ...
    for qid, rec in records.items():
        ...
        if status == "overlap":
            counts.skipped_overlap += 1
            print(f"skip overlap wd:{qid} table=hydronyms-major", file=sys.stderr)
            continue
        ...
        mapped = map_hydronym(rec, parent, today)
...
def _fetch_one_select(
    session: requests.Session,
    query: str,
) -> tuple[list[dict[str, str]], bool]:
    print("sparql hydronyms Q23397+Q4022", file=sys.stderr)
    try:
        rows = sparql_csv(session, query, timeout=90)
    except SeedError as exc:
        if not too_large_or_timeout(exc):
            raise
        print(f"sparql too_large/timeout, drop Q4022: {exc}", file=sys.stderr)
        lakes = next(typed for p31, typed in queries_for_p31(query) if p31 == LAKE_P31)
        rows = sparql_csv(session, lakes, timeout=90)
        print(f"sparql P31={LAKE_P31} rows={len(rows)}", file=sys.stderr)
        return rows, True
...
def harvest(
    *,
    ...
    place_header, existing_places = read_table(root / HYDRO_REL, PLACES_HEADER)
    decl_header, existing_decls = read_table(root / DECL_REL, DECLENSIONS_HEADER)
    if place_header and place_header != PLACES_HEADER:
        raise SeedError(f"hydronyms-major.csv: заголовок {place_header}")
    if decl_header and decl_header != DECLENSIONS_HEADER:
        raise SeedError(f"declensions/hydronyms-major.csv: заголовок {decl_header}")
    index = load_parent_index(root)
    skip_map = skip_ids(root)
    dropped_rivers = False
    if from_json is not None:
        ...
        if shard_by_region:
            rows = _fetch_sharded(session, query, root=root, started=started)
        else:
            rows, dropped_rivers = _fetch_one_select(session, query)
        if not rows:
            raise SeedError("SPARQL: пусто")
        records = _merge_rows(rows)
    records = _filter_types(records, types)
    if dropped_rivers:
        records = drop_incoming_rivers(records)
    ...
    try:
        place_payload = dump_csv_bytes_or_raise(
            PLACES_HEADER, places, "hydronyms-major.csv"
        )
        decl_payload = dump_csv_bytes_or_raise(
            DECLENSIONS_HEADER, decls, "declensions/hydronyms-major.csv"
        )
    except SeedError as exc:
        if dropped_rivers or not too_large_or_timeout(exc):
            raise
        print("too_large: drop incoming Q4022, keep lakes+seas", file=sys.stderr)
        records = drop_incoming_rivers(records)
        places, decls, applied = apply_harvest(
            records,
            index=index,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not args.dry_run:
            write_csv(root / HYDRO_REL, place_header, places)
