#!/usr/bin/env python3
"""Harvest Russian microtoponyms from Wikidata into microtoponyms.csv.

Coverage is Wikidata P31 in {урочище, овраг, поле, пещера, парк}, P17=Q159,
not GKGN. SPARQL JSON/CSV is not vendored; optional --from-csv/--from-json
...
ROOT = _ROOT
SPARQL_PATH = Path("data/raw/wikidata/microtoponyms-ru.sparql")
MICRO_REL = Path("data/curated/microtoponyms.csv")
DECL_REL = Path("data/declensions/microtoponyms.csv")
...
MICRO_SPECS: tuple[tuple[Path, int], ...] = (
    (CITIES_REL, 0),
    (MUN_REL, 1),
    (REGIONS_REL, 2),
)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Сиды микротопонимов из Wikidata SPARQL (не ГКГН)"
    )
...
def queries_for_p31(query: str) -> list[tuple[str, str]]:
    return queries_for_p31_last(query, last=("Q22698",))


def load_parent_index(root: Path) -> ParentIndex:
    return load_parent_index_specs(root, MICRO_SPECS)


def skip_ids(root: Path) -> dict[str, str]:
    return skip_ids_rels(root, MICRO_SKIP_RELS)


def map_microtoponym(
    rec: dict[str, Any],
    ...
    *,
    ...
    for qid, rec in records.items():
        ...
        if parent is None:
            ...
            continue
        mapped = map_microtoponym(rec, parent, today)
        rid = mapped["id"]
    ...
    for place in new_places:
        stub = declension_stub(place, type_code="microtoponym")
...
def harvest(
    *,
    ...
    place_header, existing_places = read_table(root / MICRO_REL, PLACES_HEADER)
    decl_header, existing_decls = read_table(root / DECL_REL, DECLENSIONS_HEADER)
    if place_header and place_header != PLACES_HEADER:
        raise SeedError(f"microtoponyms.csv: заголовок {place_header}")
    if decl_header and decl_header != DECLENSIONS_HEADER:
        raise SeedError(f"declensions/microtoponyms.csv: заголовок {decl_header}")
    ...
    place_payload = dump_csv_bytes_or_raise(PLACES_HEADER, places, "microtoponyms.csv")
    decl_payload = dump_csv_bytes_or_raise(
        DECLENSIONS_HEADER, decls, "declensions/microtoponyms.csv"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not args.dry_run:
            write_csv(root / MICRO_REL, place_header, places)
