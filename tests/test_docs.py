...
def test_readme_has_compose_one_shot() -> None:
    ...
    assert "ghcr.io/unhexx/toponym:2026.09.16" in text
...
def test_readme_has_shields_badges_and_docs_links() -> None:
    ...
    for needle in (
        ...
        "https://github.com/unhexx/toponym/releases/tag/2026.09.16",
        "docs/README.md",
...
def test_ontology_calver_matches_tagged_release() -> None:
    ...
    assert ont["project"]["calver"] == "2026.09.16"
...
def test_package_calver_aligned() -> None:
    ...
    assert 'version = "2026.09.16"' in pyproject
    assert 'version: "2026.09.16"' in citation
    ...
    assert package["version"] == "2026.09.16"
    assert PACKAGE_VERSION == "2026.09.16"
