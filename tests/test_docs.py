...
def test_readme_has_compose_one_shot() -> None:
    ...
    assert "ghcr.io/unhexx/toponym:2026.09.25" in text
...
def test_readme_has_shields_badges_and_docs_links() -> None:
    ...
    for needle in (
        ...
        "https://github.com/unhexx/toponym/releases/tag/2026.09.25",
        "docs/README.md",
...
def test_usage_guide_covers_search_and_declensions() -> None:
    ...
    assert "DEC-SEED-006" in text
    assert "DEC-SEED-007" in text
    ...
    assert "seed_agoronyms.py" in text
...
def test_changelog_unreleased_mentions_usage_and_min_update() -> None:
    ...
    assert "## [2026.09.25] - 2026-09-25" in before_loop2
    assert "## [2026.09.24] - 2026-09-24" in before_loop2
    assert "## [2026.09.17] - 2026-09-17" in before_loop2
    assert "## [2026.09.16] - 2026-09-16" in before_loop2
    assert "## [2026.09.15] - 2026-09-15" in before_loop2
    assert "## [2026.09.14] - 2026-09-14" in before_loop2
    ...
    assert "DEC-SEED-006" in before_loop2
    assert "DEC-SEED-007" in before_loop2
...
def test_ontology_calver_matches_tagged_release() -> None:
    ...
    assert ont["project"]["calver"] == "2026.09.25"
...
def test_package_calver_aligned() -> None:
    ...
    assert 'version = "2026.09.25"' in pyproject
    assert 'version: "2026.09.25"' in citation
    assert 'date-released: "2026-09-25"' in citation
    assert "Chistyakov" in citation
    assert "unhandled@exception.expert" in citation
    assert "DAO EXCEPTION EXPERT" in citation
    assert package["version"] == "2026.09.25"
    assert PACKAGE_VERSION == "2026.09.25"
...
def test_ssot_does_not_require_project_context() -> None:
    ...
    assert "2026.09.14" in spec
    assert "2026.09.15" in spec
    assert "2026.09.16" in spec
    assert "2026.09.17" in spec
    assert "2026.09.24" in spec
    assert "2026.09.25" in spec
    assert "A–L" in spec
