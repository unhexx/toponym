...
def test_required_entities_present() -> None:
    ...
    assert by_id["DEC-SEED-003"]["type"] == "Decision"
    assert by_id["DEC-SEED-003"]["status"] == "accepted"
    summary = by_id["DEC-SEED-003"]["summary"]
    assert "Q79007" in summary and "wikidata" in summary.casefold()
    assert "гар" in summary.casefold() or "фиас" in summary.casefold()
    assert by_id["DEC-SEED-001"]["status"] == "accepted"
    assert by_id["DEC-SEED-004"]["type"] == "Decision"
    assert by_id["DEC-SEED-004"]["status"] == "accepted"
    summary = by_id["DEC-SEED-004"]["summary"]
    assert "Q13626398" in summary and "wikidata" in summary.casefold()
    assert "октмо" in summary.casefold() or "гар" in summary.casefold()
    assert by_id["DEC-SEED-005"]["type"] == "Decision"
    assert by_id["DEC-SEED-005"]["status"] == "accepted"
    summary = by_id["DEC-SEED-005"]["summary"]
    assert "Q22698" in summary and "wikidata" in summary.casefold()
    assert "гкгн" in summary.casefold()
    assert by_id["DEC-SEED-001"]["status"] == "accepted"
    assert by_id["DEC-SEED-006"]["type"] == "Decision"
    assert by_id["DEC-SEED-006"]["status"] == "accepted"
    summary = by_id["DEC-SEED-006"]["summary"]
    assert "Q7930989" in summary and "wikidata" in summary.casefold()
    assert "1126" in summary or "1 126" in summary
    assert "крым" in summary.casefold() or "местн" in summary.casefold()
    assert by_id["DEC-SEED-002"]["status"] == "accepted"
    assert by_id["DEC-SEED-007"]["type"] == "Decision"
    assert by_id["DEC-SEED-007"]["status"] == "accepted"
    summary = by_id["DEC-SEED-007"]["summary"]
    assert "Q15078955" in summary and "Q532" in summary
    assert "wikidata" in summary.casefold()
    assert "гкгн" in summary.casefold() or "росстат" in summary.casefold()
    assert by_id["DEC-SEED-002"]["status"] == "accepted"
    assert by_id["DEC-SEED-008"]["type"] == "Decision"
    assert by_id["DEC-SEED-008"]["status"] == "accepted"
    summary = by_id["DEC-SEED-008"]["summary"]
    assert "Q174782" in summary and "wikidata" in summary.casefold()
    assert "омк" in summary.casefold() or "площад" in summary.casefold()
    assert by_id["DEC-SEED-002"]["status"] == "accepted"
    assert by_id["DEC-SEED-009"]["type"] == "Decision"
    assert by_id["DEC-SEED-009"]["status"] == "accepted"
    summary = by_id["DEC-SEED-009"]["summary"]
    assert "Q34442" in summary and "Q728937" in summary
    assert "wikidata" in summary.casefold()
    assert "928" in summary or "osm" in summary.casefold()
    assert by_id["DEC-SEED-010"]["type"] == "Decision"
    assert by_id["DEC-SEED-010"]["status"] == "accepted"
    summary = by_id["DEC-SEED-010"]["summary"]
    assert "Q23397" in summary and "Q4022" in summary
    assert "wikidata" in summary.casefold()
    assert "гкгн" in summary.casefold()
    assert by_id["DEC-SEED-011"]["type"] == "Decision"
    assert by_id["DEC-SEED-011"]["status"] == "accepted"
    summary = by_id["DEC-SEED-011"]["summary"]
